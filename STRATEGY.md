...
ranked by `edge_pp_of_max_loss`. The columns are:

| Column | Source field | Use |
|--|--|--|
| Grade | `data_grade` | A full size, B half size, C monitor, D blocked. |
| Signal | derived from `edge_pp_of_max_loss` + `data_grade` | Headline. Sort descending. |
...
| Edge ($) | `edge_pp_of_max_loss` | Price-space edge in % of max-loss. Bigger = better short candidate. |
