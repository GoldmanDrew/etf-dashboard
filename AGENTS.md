...
- **Jul 2026 — Bucket 4 production backtest.** Screener `bucket_4` rows (`screener_bucket`, `bucket4_net_edge_annual`, sizing/purgatory fields) pass through `build_data.py`. Nightly `scripts/build_bucket4_backtest.py` writes `data/bucket4_backtest.json` (schema `bucket4_backtest.v2`: Kelly/QCQP-weighted short-both replay with VCR cadence + sim ratchet, production `default_weights`, and compact per-pair `pair_series` paths). UI route `#/bucket-4/backtest` is an interactive book lab: custom weights reblend precomputed unit returns client-side and do not rerun Kelly/QCQP or the ratchet. Per-chart B4 Backtest uses the artifact production path when available; the legacy inverse ticker flow simulator remains available on chart pages under Backtest Flow. Policy snapshot: `config/bucket4_backtest_policy.yml`.
