...
| `bucket4_backtest.json` | `scripts/build_bucket4_backtest.py` | B4 schema v4 envelope: production replay, `default_weights`, `pair_series` daily paths, chart B4 production backtest | nightly (after full `build_data`) |
| `bucket4_pairs/{ETF}.json` | production import / research-legacy | Per-pair daily ledger; nested `inception_research`; Pair Report (plan) + chart Inception tab | with B4 build |
| `bucket4_inception_research/{ETF}.json` | `scripts/build_b4_inception_research.py` | Labeled research full-history; nested onto pair shards for `#/chart/SYM/inception` | on demand / ops |
| `bucket4_backtest_state.json` | `scripts/build_bucket4_backtest.py` | Last `h_by_pair` + sim ratchet state (diagnostic) | nightly |
| `bucket4_backtest_policy_hash.txt` | `scripts/build_bucket4_backtest.py` | Policy drift gate vs `config/bucket4_backtest_policy.yml` | nightly |
...
|---|---|---|
| `nightly.yml` | Tue–Sat 6 AM ET (10/11 UTC crons; resolve-slot picks the DST-correct slot — never gates on wall-clock hour because GitHub delivers scheduled runs hours late) | ETF metrics ingest + scoring + **full** `build_data.py` → dashboard commit + deploy. Bucket 4 export stays fail-closed and does not block that commit |
...
- **Market-obscured reverse splits still count as a jump.** When a declared reverse split coincides with a large same-day market move (NBIZ 2026-07-21 1-for-3: close ~1.87× ≈ 3× × ~−38% inverse day), `filter_splits_needing_close_basis_fix` accepts the event if a same-direction partial jump is found and residual `jump/mult` is in ~[0.45, 1.55]. Mirrored in `assets/price_basis.js`. Issuer NAV/shares confirm remains preferred when available (NBIZ 2026-06 staggered path).
...
- **Underlying split normalization (June 2026).** Corporate-actions Phase 1 now ingests splits for **ETF ∪ underlying** symbols (`linked_etfs` on underlying rows). `ingest_etf_metrics.repair_underlying_adj_close_split_basis` scales pre-split `underlying_adj_close` per underlying; `price_basis.underlying_tr_price_for_row` + `_normalize_underlying_split_sized_basis_jumps` mirror ETF split handling on the underlying leg. Decay / backtest / vol-shape / build-time gross decay pass **both** ETF and underlying split events into `buildTrSeriesFromMetrics`. `build_data.py` refuses metrics-derived `gross_decay_annual` when `underlying_split_suspect` or extreme screener disagreement (sign or magnitude). `audit_split_tr_quality.py --audit-underlying-cliffs` and `audit_dashboard_data_quality.py` block unexplained underlying cliffs on bucket-1 names.
...
- **Sep 2026 — Nightly no longer dies on the Bucket 4 export.** `b4_export` is `continue-on-error`. Split-TR, the dashboard commit, and Pages deploy still run. The job is failed again after the commit so the export error stays red. `data/bucket4_*` is not replaced with a simulated book. Afternoon freshness reconciles REX AUM to NAV × shares (`repair_rex_session_nav_close.py --reconcile-only`) before the hard gate. Nightly and the afternoon build install `requirements-research.txt` so borrow scoring and Bucket 5 can import scikit-learn and lightgbm.
...
- **Jul 2026 — B4 Inception chart tab + fleet panel sanitize.** Listing→latest research is a ChartPage **Inception** tab (`#/chart/SYM/inception`) for B4 membership ETFs; B4 Pairs / Pair Report default to the **plan/book ledger**. `sanitize_panel_vs_session_close` replaces ETF `a_px` on day cliffs or early/late `panel/close` ratio steps, preferring `etf_adj_close` when it removes reverse-split cliffs. Rebuild via `scripts/build_b4_inception_research.py`; soft audit `scripts/audit_b4_inception_paths.py`. Plan-path wipe banners (CONI/LITZ/RKLZ) require ls-algo re-export — do not rewrite production ledgers in the dashboard. Ops: [`docs/b4_archive_extension_ops.md`](docs/b4_archive_extension_ops.md).
- **Jul 2026 — B4 Pair Report UX + dual-path history.** Pair Report title uses spaced `ETF vs UND`; toolbar is left-aligned; tables use `.pair-report-table`. KPI CAGR is sleeve-equity compounds (not PnL÷Notional); gross leg is \|ETF\|+\|und\| short exposure scaled from `notional_basis_usd`. Meta shows **Plan entered** (membership/ledger) vs **ETF inception** (metrics). Prefer exported `rebalance_log` (`rebalance_log_basis=production_execution_ledger`); if empty/stub while `daily.rebalance` has flags, UI synthesizes via `rebalanceLogFromDaily` (`source=daily_derived`) and importer adds a soft limitation. Nested `inception_research` feeds the chart Inception tab (Pair Report may preview). Acceptance: [`docs/bucket4_pair_report_acceptance.md`](docs/bucket4_pair_report_acceptance.md).
- **Jul 2026 — Bucket 4 authoritative production replay.** Production B4 is a strict two-repo contract. `ls-algo/scripts/export_b4_dashboard.py` packages the full `production_actual_backtest` GTP and execution ledgers as `bucket4_production_replay.v1`, including source/config/input hashes and pair-to-sleeve reconciliation. Dashboard `scripts/build_bucket4_backtest.py` defaults to importing that contract through `scripts/import_bucket4_production.py` and writes `bucket4_backtest.v4`; it must fail closed when the export is missing, dirty, tampered, or unreconciled. The old dashboard sizing/pair simulator is available only through `--mode research-legacy` and must never be used as a production fallback. B4 Book and Pair Report render exported ledger PnL; client custom reblending is disabled for v4. Editable research remains on chart **Backtest** and **Drip** surfaces. `config/bucket4_backtest_policy.yml` is research-only and is not a production policy source.
...
This repo participates in the **borrow-etf** Memory Mesh (shared with `ls-algo` / `etf-dashboard`).

- Graph: `C:\Users\drewg\Projects\.agent-memory\GRAPH.md`
- Local junction: `.agent-memory/` ? shared hub (`CONTEXT.md`, `MEMORY.md`, `bridges.md`, `daily/`)
- Skill policy: `C:\Users\drewg\Projects\.agent-memory\SKILL_POLICY.md`
- Use `/repo-memory` to read/update hub notes. Propose durable items as `[PROPOSED]` in `daily/`; do not silently edit `MEMORY.md`.
