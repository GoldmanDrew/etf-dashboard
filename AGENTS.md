...
| `etf_metrics_daily.{parquet,csv,json}` | `ingest_etf_metrics.py` | Stats tab (NAV/AUM/shares chart); `underlying_adj_close` via chunked Yahoo (`ETF_METRICS_UNDERLYING_YF_CHUNK_SIZE`) plus **gap backfill** (per-underlying Yahoo fetch for rows still null after the main merge) | daily 5 AM ET |
