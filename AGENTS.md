...
| `bucket4_backtest.json` | `scripts/build_bucket4_backtest.py` | `#/bucket-4/backtest` portfolio replay + chart B4 h defaults | nightly (after full `build_data`) |
...
- **Jul 2026 — Bucket 4 production backtest.** Screener `bucket_4` rows (`screener_bucket`, `bucket4_net_edge_annual`, sizing/purgatory fields) pass through `build_data.py`. Nightly `scripts/build_bucket4_backtest.py` writes `data/bucket4_backtest.json` (Kelly/QCQP-weighted short-both replay with VCR cadence + sim ratchet). UI route `#/bucket-4/backtest` replaces the legacy add-only Backtest Flow page; per-chart backtest prefills **h** from the artifact for B4 rows. Policy snapshot: `config/bucket4_backtest_policy.yml`.
