...
│   ├── assets/pair_backtest.js      (`simulateInversePairBacktest`: notional hedge `shortMV/longMV = h`, gross split `G/(1+h)` & `h·G/(1+h)`; legacy `runPairBacktest` two-ETF bucket pair)
